...
SELECT order_external_id,
       ...
       IF(fk_triggered_customer_breach_flag = 1
          OR fk_triggered_promise_change_flag = 1
          OR customer_contact_flag = 1
          OR fk_triggered_cancellation_flag = 1
          OR fk_triggered_return_flag = 1, 0, 1) AS perfect_delivery_flag,
       slot_adherence_flag,
       slot_change_reason,
       asset,
       (CASE
            WHEN asset IN('12_Returned',
                          '11_Delivered',
                          '13_Cancelled') THEN 0
            ...
            ELSE 0
        END) AS asset_breach_flag,
       shipment_last_consignment_create_datetime,
       lookup_date(to_Date(perfect_order_final_promised_date)) AS perfect_order_final_promised_date_key,
       IF(calculated_delivery_hub LIKE '%3PL%'
          OR shipment_carrier = '3PL', '3PL', 'FSD') AS calculated_carrier,
       calculated_delivery_hub,
       order_pincode_key,
       slot_booking_ref_id,
       cu_sub_issue_type,
       incident_creation_time,
       DATEDIFF(to_date(order_item_unit_init_promised_datetime), to_date(order_item_max_approved_time)) AS ndd_flag,
       ...
       IF(pd_new_customer_breach = 1
          OR fk_triggered_promise_change_flag = 1
          OR pd_new_fk_triggered_rto = 1, 0, 1) AS pd_new_flag,
       IF(customer_contact_flag = 1
          OR pe_shipment_condition_breach = 1
          OR pe_sla_breach = 1
          OR pe_delivery_attempt_breach = 1
          OR pe_delivery_precision_breach = 1, 0, 1) AS pe_flag,
       return_date,
       vendor_name,
...
FROM
  (SELECT order_external_id,
          order_id,
          IF(isnull(shipment_last_consignment_create_datetime), 0, IF((last_conn_cutoff_in_sec - HOUR(shipment_last_consignment_create_datetime) * 3600 - MINUTE(shipment_last_consignment_create_datetime) * 60 - SECOND(shipment_last_consignment_create_datetime)) > 0, (last_conn_cutoff_in_sec - HOUR(shipment_last_consignment_create_datetime) * 3600 - MINUTE(shipment_last_consignment_create_datetime) * 60 - SECOND(shipment_last_consignment_create_datetime)), 0)) AS tpt_time_diff_in_sec,
          (CASE
               WHEN order_item_status IN('returned',
                                         'return_requested')
                    OR ekl_shipment_type IN('approved_rto',
                                            'unapproved_rto') THEN '12_Returned'
               WHEN order_item_status IN('delivered')
                    OR shipment_current_status IN('delivered',
                                                  'Delivered')
               ...
               WHEN order_item_status IN('created',
                                         'on_hold') THEN '01_OMS'
           ...
           END) AS asset,
          from_unixtime(unix_timestamp(IF(fulfill_item_unit_dispatch_expected_time > fulfill_item_unit_dispatch_actual_time
                                          OR isnull(fulfill_item_unit_dispatch_actual_time), fulfill_item_unit_dispatch_expected_time, fulfill_item_unit_dispatch_actual_time)) + 7200) AS mh_cutoff_datetime,
          DA.order_item_id,
          order_item_unit_tracking_id,
          order_item_unit_shipment_id,
          tracking_id_type,
          ...
          shipment_pending_age,
          rvp_pickup_breach_flag,
          IF(to_date(ekl_delivery_datetime) <= to_date(order_item_unit_final_promised_date_use), 'Delivered by promise', IF(to_date(shipment_first_ofd_datetime) <= to_date(order_item_unit_final_promised_date_use), 'Customer_Dependency-Attempted Undelivered', IF(fulfill_item_unit_dispatch_actual_time > fulfill_item_unit_dispatch_expected_time, fc_breach_bucket, IF(to_date(logistics_promise_datetime) > to_date(order_item_unit_final_promised_date_use), 'LP greater than CP', IF(NOT isnull(fulfill_item_unit_dispatch_actual_time), shipment_breach_bucket, IF(order_item_status = 'on_hold', 'on_hold', 'rca')))))) AS customer_breach_breakup,
          IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto')
             AND NOT isnull(end_state_datetime)
             AND NOT isnull(shipment_rto_create_time), datediff(to_date(end_state_datetime), to_date(shipment_rto_create_time)), IF(ekl_shipment_type = 'rvp'
                                                                                                                                    AND NOT isnull(end_state_datetime)
                                                                                                                                    AND NOT isnull(shipment_created_at_datetime), datediff(to_date(end_state_datetime), to_date(shipment_created_at_datetime)), NULL)) AS rvp_rto_reconciliation_days,
          IF(ekl_shipment_type = 'rvp', IF(NOT isnull(fsd_last_dhhub_sent_datetime), IF(to_date(fsd_last_dhhub_sent_datetime) <= shipment_dh_rvp_promised_date, 0, 1), IF(datediff(present_day, shipment_dh_rvp_promised_date) > 0, 1, 0)), NULL) AS shipment_dh_rvp_breach_flag,
          IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto')
             AND NOT isnull(fsd_assignedhub_received_time), IF(NOT isnull(fsd_last_dhhub_sent_datetime), IF(to_date(fsd_last_dhhub_sent_datetime) <= shipment_dh_rto_promised_date, 0, 1), IF(datediff(present_day, shipment_dh_rto_promised_date) > 0, 1, 0)), NULL) AS shipment_dh_rto_breach_flag,
          IF(ekl_shipment_type IN('forward', 'unapproved_rto', 'approved_rto')
             AND NOT isnull(fsd_assignedhub_received_time), IF(ekl_shipment_type = 'forward', IF(isnull(ekl_delivery_datetime), 'Forward_Intransit', 'Forward_Delivered'), IF(isnull(fsd_last_dhhub_sent_datetime), 'RTO_In_Hub', 'RTO_Intransit_To_DC')), NULL) AS forward_dh_reconciliation_bucket,
          IF(ekl_shipment_type = 'rvp'
             AND shipment_carrier = 'FSD', IF(shipment_current_status IN('not_received', 'Not_Received'), 'RVP_cancelled', IF(isnull(rvp_pickup_complete_datetime), 'RVP_Pickup_Pending', IF(isnull(fsd_last_dhhub_sent_datetime), 'RVP_In_Hub', 'Intransit_To_DC'))), NULL) AS reverse_dh_reconciliation_bucket,
          IF(ekl_shipment_type = 'rvp'
             AND shipment_current_status IN('not_received', 'Not_Received'), 1, 0) AS rvp_cancelled_flag,
          datediff(to_date(order_item_unit_init_promised_datetime), to_date(order_item_created_at)) AS existing_sla_days,
          IF(ekl_shipment_type = 'forward'
             AND NOT isnull(ekl_delivery_datetime), datediff(to_date(ekl_delivery_datetime), to_date(order_item_created_at)), NULL) AS performance_sla_days,
          datediff(to_date(order_item_unit_final_promised_date_use), to_date(fulfill_item_unit_dispatch_expected_time)) AS existing_d2d_days,
          IF(ekl_shipment_type = 'forward'
             AND NOT isnull(ekl_delivery_datetime), IF(shipment_carrier = '3PL', datediff(to_date(ekl_delivery_datetime), to_date(vendor_dispatch_datetime)), datediff(to_date(ekl_delivery_datetime), to_date(fulfill_item_unit_dispatch_expected_time))), NULL) AS performance_d2d_days,
          IF(ekl_shipment_type = 'forward'
             AND isnull(ekl_delivery_datetime), datediff(present_day, to_Date(order_item_unit_final_promised_date_use)), NULL) AS pending_beyond_customer_promise_age,
          datediff(to_date(order_item_unit_final_promised_date_use), present_day) AS customer_promise_proactive_age,
          IF(ekl_shipment_type = 'rvp', IF(pincode_location_type IN('ESS', 'Fran'), IF(NOT isnull(end_state_datetime), IF(to_date(end_state_datetime) <= date_add(to_date(shipment_created_at_datetime), 14), 0, 1), IF(datediff(present_day, date_add(to_date(shipment_created_at_datetime), 14)) > 0, 1, 0)), IF(NOT isnull(end_state_datetime), IF(to_date(end_state_datetime) <= date_add(to_date(shipment_created_at_datetime), 10), 0, 1), IF(datediff(present_day, date_add(to_date(shipment_created_at_datetime), 10)) > 0, 1, 0))), IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto'), IF(pincode_location_type IN('ESS', 'Fran'), IF(NOT isnull(end_state_datetime), IF(to_date(end_state_datetime) <= date_add(to_date(shipment_rto_create_time), 10), 0, 1), IF(datediff(present_day, date_add(to_date(shipment_rto_create_time), 10)) > 0, 1, 0)), IF(NOT isnull(end_state_datetime), IF(to_date(end_state_datetime) <= date_add(to_date(shipment_rto_create_time), 7), 0, 1), IF(datediff(present_day, date_add(to_date(shipment_rto_create_time), 7)) > 0, 1, 0))), NULL)) AS reverse_breach_flag,
          IF(ekl_shipment_type = 'rvp', lookup_date(date_add(to_date(shipment_created_at_datetime), 10)), IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto'), lookup_date(date_add(to_date(shipment_rto_create_time), 7)), NULL)) AS rev_promise_date_key,
          perfect_order_breach_bucket,
          return_type,
          return_date,
          return_reason,
          ...
          customer_contact_issue_type,
          customer_contact_sub_sub_issue_type,
          IF(ekl_shipment_type = 'forward', IF(ekl_delivery_datetime IS NOT NULL, IF(to_date(ekl_delivery_datetime) <= to_date(perfect_order_final_promised_date), 0, IF(to_date(shipment_first_ofd_datetime) <= to_date(perfect_order_final_promised_date)
                                                                                                                                                                         OR to_date(tpl_first_ofd_time) <= to_date(perfect_order_final_promised_date), 0, IF(shipment_breach_bucket IN('Customer_Dependency-Attempted Undelivered', 'Customer_Dependency-Attempted Undelivered-3PL'), 0, 1))), IF(to_date(shipment_first_ofd_datetime) <= to_date(perfect_order_final_promised_date)
                                                                                                                                                                                                                                                                                                                                                                                                  OR to_date(tpl_first_ofd_time) <= to_date(perfect_order_final_promised_date), 0, IF(to_date(perfect_order_final_promised_date) < present_day, 1, 0))), 0) AS fk_triggered_customer_breach_flag,
          fk_triggered_promise_change_flag,
          fk_triggered_cancellation_flag,
          fk_triggered_return_flag,
          IF((ekl_delivery_datetime <= fulfill_item_unit_delivered_status_expected_time
              AND ekl_delivery_datetime >= fulfill_item_unit_deliver_after_time), 1, 0) AS slot_adherence_flag,
          slot_change_reason,
          ...
          CASE
              WHEN ekl_shipment_type = 'forward'
                   AND order_item_unit_is_promised_date_updated = 1
                   AND slot_changed_by = 'cust'
                   AND COALESCE(shipment_first_ofd_date_key,unix_timestamp()) <= order_item_unit_final_promised_date_key THEN 0
              WHEN ekl_shipment_type = 'forward'
                   AND shipment_breach_bucket NOT IN('delivered by logistics promise date',
                                                     'Customer_Dependency-Attempted Undelivered',
                                                     'Customer_Dependency-Attempted',
                                                     'Customer_Dependency-Attempted Undelivered-3PL') THEN 1
          ...
          CASE
              WHEN ekl_shipment_type IN('approved_rto',
                                        'unapproved_rto')
                   AND return_reason IN('Damaged Shipment',
          ...
          CASE
              WHEN return_reason IN('Damaged Shipment',
                                    'DAMAGED_MISSING_PRODUCT',
                                    'DAMAGED_PRODUCT',
                                    'MISSHIPMENT',
                                    'MISSING_ITEM') THEN 1
          ...
          CASE
              WHEN ekl_shipment_type = 'forward'
                   AND datediff(to_date(ekl_delivery_datetime), to_date(order_item_created_at)) > 4 THEN 1
              ELSE 0
          ...
          CASE
              WHEN ekl_shipment_type = 'forward'
                   AND datediff(to_date(order_item_unit_final_promised_date_use), to_date(ekl_delivery_datetime)) > 2 THEN 1
              ELSE 0
          END AS pe_delivery_precision_breach,
          vendor_name,
          IF(ekl_shipment_type = 'forward'
             AND NOT isnull(ekl_delivery_datetime), datediff(to_date(ekl_delivery_datetime), to_date(order_item_created_at)), NULL) AS order_create_to_delivery_sla_days,
          picklist_display_id,
          picklist_created_at,
          picklist_item_confirmed_at
   FROM
     (SELECT C.runsheet_close_datetime,
             ...
             IF(ekl_delivery_datetime IS NOT NULL, IF(to_date(ekl_delivery_datetime) <= to_date(perfect_order_final_promised_date), 'PO_Delivered_by_promise', 'PO_breached_delivered'), IF(to_date(perfect_order_final_promised_date) < present_day, 'PO_pending_breached', 'PO_pending_not_breached')) AS perfect_order_breach_bucket,
             IF(order_item_unit_is_promised_date_updated = 1
                AND slot_changed_by = 'ekl', 1, 0) AS fk_triggered_promise_change_flag,
             (CASE
                  WHEN cancellation_derived_reason IN('Seller',
                                                      'MarketPlace')
                       AND cancellation_reason <> 'auto_decline_verification' THEN 1
                  ELSE 0
              END) AS fk_triggered_cancellation_flag,
             IF(return_derived_reason = 'Flipkart', 1, 0) AS fk_triggered_return_flag,
             IF(order_item_status IN('cancelled', 'created', 'returned', 'retuned')
                OR shipment_pending_location = 'not_pending', 0, 1) AS order_pending_flag,
             IF(order_item_status IN('cancelled', 'created', 'returned', 'retuned'), order_item_status, IF(order_item_status = 'on_hold', CONCAT('on_hold_', IF(NOT(isnull(call_verification_reason)), call_verification_reason, 'no_reason')), IF(shipment_pending_location = 'not_pending', 'not_pending', IF(isnull(fulfill_item_unit_reserved_status_b2c_actual_time)
                                                                                                                                                                                                                                                                                                                AND tracking_id_type = 'forward', 'reservation_pending', IF(IsNull(order_item_unit_shipment_id)
                                                                                                                                                                                                                                                                                                                                                                            AND tracking_id_type = 'forward', 'dispatch_pending', shipment_pending_location))))) AS order_final_status,
             IF(ekl_shipment_type = 'forward'
                OR isnull(ekl_shipment_type), IF(order_item_status IN('on_hold', 'created'), 'CS', IF(order_item_status = 'approved'
                                                                                                      AND isnull(fulfill_item_unit_reserved_status_b2c_actual_time), 'TECH', IF(order_item_status = 'approved'
                                                                                                                                                                                AND NOT(isnull(fulfill_item_unit_reserved_status_b2c_actual_time)), 'WareHouse', shipment_pending_location))), IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto', 'rvp'), shipment_pending_location, 'RCA')) AS order_pending_location,
             IF(order_item_status IN('cancelled', 'created', 'returned', 'retuned')
                OR shipment_pending_location = 'not_pending', 'not_pending', datediff(present_day, to_date(order_item_created_at))) AS order_pending_age,
             IF(to_date(ekl_delivery_datetime) <= to_date(logistics_promise_datetime), 'delivered by logistics promise date', IF(shipment_carrier = 'FSD', IF(to_date(shipment_first_ofd_datetime) <= to_date(logistics_promise_datetime), IF(fsd_first_undel_customer_dependency_flag = 1, 'Customer_Dependency-Attempted Undelivered', 'RCA'), IF(mh_breach_flag = 1, 'MotherHub_Breach', IF(tpt_intransit_breach_flag = 1, 'Transport_Breach', IF(shipment_dh_fwd_breach_flag = 1, 'Breach-EKL_DH', IF(shipment_current_status = 'Undelivered_OutOfDeliveryArea', 'Breach-EKL_DH_ODA', IF(fsd_current_customer_dependency_flag = 1, 'Customer_Dependency-Attempted', 'RCA')))))), IF(to_date(tpl_first_ofd_time) <= to_date(order_item_unit_final_promised_date_use), 'Customer_Dependency-Attempted Undelivered-3PL', 'Breach-3PL'))) AS shipment_breach_bucket,
             IF(shipment_carrier = 'FSD', IF(to_date(ekl_delivery_datetime) <= to_date(order_item_unit_final_promised_date_use), 0, 1), IF(to_date(ekl_delivery_datetime) <= to_date(order_item_unit_final_promised_date_use), 0, 1)) AS shipment_breach_flag,
             IF(ekl_pending_flag = 'pending', IF(ekl_shipment_type IN('unapproved_rto', 'approved_rto'), datediff(present_day, to_date(shipment_rto_create_time)), datediff(present_day, to_date(shipment_created_at_datetime))), - 100) AS shipment_pending_age,
             IF(ekl_shipment_type = 'rvp', IF(to_date(rvp_pickup_complete_datetime) <= date_add(to_date(shipment_created_at_datetime), 2), 0, 1), NULL) AS rvp_pickup_breach_flag,
             slot_change_reason,
             last_conn_cutoff_in_sec,
             shipment_last_consignment_create_datetime,
             shipment_last_consignment_eta_in_sec,
      ...
      FROM
        (SELECT B.shipment_dh_fwd_promised_date,
                ...
                IF(tracking_id_type = 'forward'
                   AND ekl_shipment_type = 'forward', IF(to_date(ekl_delivery_datetime) <= shipment_dh_fwd_promised_date, 0, IF(shipment_dh_pending_flag = 0, 'not@DH', IF(fsd_first_undel_customer_dependency_flag = 1, 'Customer_Dependency-Attempted Undelivered', 1))), 'not_fwd_shipment') AS shipment_dh_fwd_breach_flag,
                IF(tracking_id_type = 'forward', IF(shipment_dh_pending_flag = 0, - 100, datediff(present_day, to_date(fsd_assignedhub_received_time))), IF(shipment_dh_pending_flag = 0, - 100, datediff(present_day, to_date(rvp_pickup_complete_datetime)))) AS shipment_dh_pending_age,
                IF(tracking_id_type = 'forward', IF(NOT isnull(ekl_delivery_datetime)
                                                    AND fsd_number_of_ofd_attempts = 1, 1, 0), NULL) AS first_attempt_delivery_flag,
                IF(ekl_shipment_type = 'forward'
                   AND NOT isnull(ekl_delivery_datetime)
                   AND NOT isnull(fsd_assignedhub_received_time), datediff(to_date(ekl_delivery_datetime), to_date(fsd_assignedhub_received_time)), NULL) AS shipment_dh_days_for_delivery,
                IF(ekl_pending_flag = 'not_pending', 'not_pending', IF(ekl_shipment_type = 'rvp'
                                                                       AND shipment_carrier = 'FSD'
                                                                       AND isnull(rvp_pickup_complete_datetime), 'RVP_pickup_pending', IF(shipment_carrier = 'VNF', 'VNF', IF(shipment_dh_pending_flag = 1, 'DH_pending', IF(upper(shipment_current_status) = 'OUT_FOR_DELIVERY', 'Out_For_Delivery', IF(shipment_current_status = 'Expected', 'Intransit', IF(ekl_shipment_type = 'forward', IF(isnull(shipment_inscan_time), 'MH_inscan_pending', IF(shipment_carrier = '3PL', IF(shipment_current_status = 'received', '3PL_Central_Hub_Pending', '3PL_pending'), IF(isnull(shipment_first_consignment_id), 'consignment_creation_pending', fsd_last_current_hub_type))), IF(shipment_current_status IN('Received_By_Ekl', 'Returned_To_Ekl'), 'Central_Hub_Pending', IF(shipment_carrier = '3PL', IF(shipment_current_status IN('dispatched_to_merchant', 'received_by_merchant', 'marked_for_merchant_dispatch', 'received_with_error'), fsd_last_current_hub_type, '3PL_pending'), fsd_last_current_hub_type))))))))) AS shipment_pending_location,
                IF(ekl_shipment_type = 'rvp', IF(NOT isnull(rvp_pickup_complete_datetime)
                                                 AND shipment_rvp_pk_number_of_attempts = 1, 1, 0), NULL) AS first_attempt_pickup_flag,
                ...
                lookup_date(to_date(rvp_pickup_complete_datetime)) AS rvp_pickup_complete_date_key,
                lookup_date(to_date(end_state_datetime)) AS end_state_date_key,
                lookup_date(to_date(fsd_last_dhhub_sent_datetime)) AS fsd_last_dhhub_sent_date_key,
                lookup_date(order_item_unit_final_promised_date_use) AS order_item_unit_final_promised_date_use_date_key,
                lookup_date(to_date(order_item_created_at)) AS order_item_created_at_date_key,
                lookup_date(to_date(fulfill_item_unit_ship_expected_time)) AS fulfill_item_unit_ship_expected_date_key,
                lookup_date(to_date(shipment_inscan_time)) AS shipment_inscan_date_key,
                lookup_date(to_date(shipment_rto_create_time)) AS shipment_rto_create_date_key,
                lookup_date(to_date(fsd_assignedhub_received_time)) AS fsd_assignedhub_received_date_key,
                lookup_date(to_date(shipment_first_ofd_datetime)) AS shipment_first_ofd_date_key,
                lookup_date(to_date(shipment_last_ofd_datetime)) AS shipment_last_ofd_date_key,
                lookup_date(to_date(vendor_dispatch_datetime)) AS vendor_dispatch_date_key,
                lookup_date(to_date(tpl_first_ofd_time)) AS tpl_first_ofd_date_key,
                lookup_date(to_date(tpl_last_ofd_time)) AS tpl_last_ofd_date_key,
                lookup_date(to_date(fsd_assigned_hub_sent_datetime)) AS fsd_assigned_hub_sent_date_key,
                lookup_date(to_date(openbox_reject_datetime)) AS openbox_reject_date_key,
         ...
         FROM
           (SELECT A.order_item_unit_id,
                   ...
                   IF(tracking_id_type = 'forward', IF(to_date(fsd_assignedhub_received_time) <= to_date(order_item_unit_final_promised_date_use), to_date(order_item_unit_final_promised_date_use), to_date(fsd_assignedhub_received_time)), NULL) AS shipment_dh_fwd_promised_date,
                   IF(ekl_shipment_type = 'rvp'
                      AND NOT IsNull(rvp_pickup_complete_datetime), (CASE
                                                                         WHEN rvp_hub_id IN(592, 497, 498, 590, 591, 500, 593) THEN date_add(to_date(rvp_pickup_complete_datetime), 2)
                                                                         WHEN rvp_hub_id IN(595) THEN date_add(to_date(rvp_pickup_complete_datetime), 3)
                                                                         ELSE date_add(to_date(rvp_pickup_complete_datetime), 1)
                                                                     END), NULL) AS shipment_dh_rvp_promised_date,
                   IF(tracking_id_type = 'forward', IF(ekl_shipment_type IN('approved_rto', 'unapproved_rto'), date_add(to_date(shipment_rto_create_time), 1), NULL), NULL) AS shipment_dh_rto_promised_date,
                   IF(fsd_last_current_hub_type IN('BULK_HUB', 'DELIVERY_HUB')
                      AND shipment_current_status <> 'Expected', 1, 0) AS shipment_dh_pending_flag,
                   IF(NOT(isnull(ekl_delivery_datetime))
                      OR NOT isnull(end_state_datetime)
                      OR shipment_current_status IN('not_received', 'reverse_pickup_cancelled', 'PICKUP_Cancelled', 'NotPicked_Attempted_CustomerHappyWithProduct', 'expected', 'dispatch_failed', 'Delivered', 'delivered', 'reshipped', 'delivered', 'returned_to_seller', 'received_by_merchant', 'damaged', 'returned', 'Undelivered_UntraceableFromHub', 'lost', 'Lost', 'Not_Received'), 'not_pending', 'pending') AS ekl_pending_flag,
                   to_date(from_unixtime(unix_timestamp())) AS present_day,
                   IF(order_item_status = 'created', 'not_approved', IF(order_item_status = 'cancelled', 'cancelled', IF(order_item_status IN('returned', 'retuned', 'return_requested')
                                                                                                                         OR ekl_shipment_type IN('approved_rto', 'unapproved_rto', 'rvp'), 'reverse', 'forward'))) AS order_final_unit_type,
                   IF(fsd_firstundeliverystatus IN('Undelivered_Attempted', 'Undelivered_COD_Not_Ready', 'Undelivered_Customer_Not_Available', 'Undelivered_Door_Lock', 'Undelivered_Incomplete_Address', 'Undelivered_No_Response', 'Undelivered_NonServiceablePincode', 'Undelivered_Order_Rejected_By_Customer', 'Undelivered_Order_Rejected_OpenDelivery', 'Undelivered_OutOfDeliveryArea', 'Undelivered_Request_For_Reschedule'), 1, 0) AS fsd_first_undel_customer_dependency_flag,
                   IF(shipment_current_status IN('Undelivered_Attempted', 'Undelivered_COD_Not_Ready', 'Undelivered_Customer_Not_Available', 'Undelivered_Door_Lock', 'Undelivered_Incomplete_Address', 'Undelivered_No_Response', 'Undelivered_NonServiceablePincode', 'Undelivered_Order_Rejected_By_Customer', 'Undelivered_Order_Rejected_OpenDelivery', 'Undelivered_OutOfDeliveryArea', 'Undelivered_Request_For_Reschedule'), 1, 0) AS fsd_current_customer_dependency_flag,
                   from_unixtime(unix_timestamp(CONCAT(logistics_promise_date_key, SUBSTR(LPAD(logistics_promise_time_key, 4, '0'), 1, 4)), 'yyyyMMddHHmm')) AS logistics_promise_datetime,
                   shipment_created_at_datetime,
                   shipment_last_consignment_create_datetime,
                   ...
                   IF(isnull(fulfill_item_unit_dispatch_actual_time), IF(from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time), 'yyyy-MM-dd') < from_unixtime(unix_timestamp(), 'yyyy-MM-dd'), 'pending_without_breach', 'pending_with_breach'), 'not_pending') AS fc_pending_flag,
                   IF(from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time), 'yyyy-MM-dd') < from_unixtime(unix_timestamp(), 'yyyy-MM-dd'), IF(isnull(fulfill_item_unit_dispatch_actual_time), datediff(from_unixtime(unix_timestamp(), 'yyyy-MM-dd'), from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time), 'yyyy-MM-dd')) - 1, NULL), NULL) AS fc_pending_age,
                   IF(isnull(fulfill_item_unit_dispatch_actual_time), IF(from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time), 'yyyy-MM-dd') < from_unixtime(unix_timestamp(), 'yyyy-MM-dd'), 1, 0), IF(fulfill_item_unit_dispatch_expected_time < fulfill_item_unit_dispatch_actual_time, 1, 0)) AS fc_breach_flag,
                   IF(isnull(fulfill_item_unit_dispatch_actual_time) AND(from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time), 'yyyy-MM-dd') >= from_unixtime(unix_timestamp(), 'yyyy-MM-dd')), 'In-Progress', IF(fulfill_item_unit_dispatch_expected_time >= fulfill_item_unit_dispatch_actual_time, 'FC_fulfilled', IF(from_unixtime(unix_timestamp(order_item_max_on_hold_time) +(5 * 60 * 30)) > from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time)), 'FC_CS_Breach', IF(from_unixtime(unix_timestamp(fulfill_item_unit_final_reserved_datetime) +(5 * 60 * 30)) > from_unixtime(unix_timestamp(fulfill_item_unit_dispatch_expected_time)), 'FC_Fulfilment_Tech_Breach', 'FC_Operations_Breach')))) AS fc_breach_bucket,
                   IF(slot_changed_by = 'cust', order_item_unit_final_promised_date_use, order_item_unit_init_promised_datetime) AS perfect_order_final_promised_date,
                   last_conn_cutoff_in_sec,
                   slot_change_reason,
                   fulfill_item_unit_final_reserved_datetime,
            ...
            FROM
              (SELECT order_tracking.order_external_id,
                      ...
                      IF(isnull(ff.fulfill_item_unit_reserved_status_b2b_actual_time), IF(isnull(ff.fulfill_item_unit_reserved_status_b2c_actual_time), NULL, ff.fulfill_item_unit_reserved_status_b2c_actual_time), IF(isnull(ff.fulfill_item_unit_reserved_status_b2c_actual_time), ff.fulfill_item_unit_reserved_status_b2b_actual_time, IF(ff.fulfill_item_unit_reserved_status_b2b_actual_time < ff.fulfill_item_unit_reserved_status_b2c_actual_time, ff.fulfill_item_unit_reserved_status_b2b_actual_time, ff.fulfill_item_unit_reserved_status_b2c_actual_time))) AS fulfill_item_unit_final_reserved_datetime
               FROM
                 (SELECT DISTINCT subset_1.order_external_id,
                                  subset_1.order_id,
                  ...
                  FROM
                    (SELECT prod_1.product_categorization_hive_dim_key,
                     ...
                     LEFT OUTER JOIN
                       (SELECT DISTINCT order_item_id,
                        ...
                        WHERE order_item_unit_status NOT IN('created') ) oiu_1 ON prod_1.product_categorization_hive_dim_key = oiu_1.order_item_product_id_key) subset_1
                  UNION ALL SELECT DISTINCT oms_fct.order_external_id,
                                            oms_fct.order_id,
                  ...
                  FROM bigfoot_snapshot.dart_fkint_scp_rrr_return_4_0_view_total RETURN
                  LEFT OUTER JOIN bigfoot_snapshot.dart_fkint_scp_rrr_return_item_3_0_view_total return_item ON RETURN.`data`.return_id_string = return_item.`data`.return_id_string
                  JOIN bigfoot_external_neo.scp_oms__la_oms_fact oms_fct ON oms_fct.order_item_id = return_item.`data`.order_item_id_string
                  JOIN bigfoot_external_neo.sp_product__product_categorization_hive_dim prod_dim ON oms_fct.order_item_product_id_key = prod_dim.product_categorization_hive_dim_key
                  WHERE prod_dim.is_large = 1
                    AND RETURN.`data`.return_type = 'customer_return'
                    AND return_item.`data`.tracking_id IS NOT NULL
                    UNION ALL
                    SELECT DISTINCT subset_3.order_external_id,
                                    subset_3.order_id,
                    ...
                    FROM
                      (SELECT prexo_ex.product_categorization_hive_dim_key AS product_categorization_hive_dim_key,
                       ...
                       FROM
                         (SELECT `data` . pickup_id,
                                 `data` . shipment_id,
                                 `data` . tracking_id,
                                 `data` . latched_shipment_id
                          FROM bigfoot_snapshot.dart_fkint_scp_ro_pickup_1_view_total
                          WHERE `data` . pickup_to_address_id IN('blr_jig_01L',
                                                                 'blr_jig_03L',
                                                                 'del_lhr_03L',
                                                                 'del_tik_01L',
                                                                 'kol_dan_01L',
                                                                 ...
                                                                 'mum_chm_01L') ) prexo_p
                       INNER JOIN
                         (SELECT `data` . ref_entity_id,
                                 `data` . ref_entity,
                                 `data` . pickup_id
                          FROM bigfoot_snapshot.dart_fkint_scp_ro_pickup_item_1_view_total
                          WHERE `data` . ref_entity = 'product_exchange_item' ) prexo_pi ON prexo_p.pickup_id = prexo_pi.pickup_id
                       LEFT JOIN
                         (SELECT `data` . order_item_id,
                                 `data` . product_id,
                                 `data` . product_exchange_item_id,
                                 lookupkey('product_id', `data` . product_id) AS product_categorization_hive_dim_key
                          FROM bigfoot_snapshot.dart_fkint_scp_rrr_product_exchange_item_1_view_total) prexo_ex ON prexo_pi.ref_entity_id = prexo_ex.product_exchange_item_id
                       LEFT OUTER JOIN
                         (SELECT order_external_id,
                                 order_id,
                                 order_item_unit_shipment_id AS unit_shipment_id
                          FROM bigfoot_external_neo.scp_oms__la_oms_fact) prexo_o ON prexo_o.unit_shipment_id = prexo_p.latched_shipment_id) subset_3 WHERE NOT ISNULL(subset_3.order_item_unit_tracking_id) ) order_tracking
               LEFT OUTER JOIN bigfoot_external_neo.scp_oms__la_oms_fact oms ON order_tracking.order_item_unit_id = oms.order_item_unit_id
               LEFT OUTER JOIN bigfoot_external_neo.scp_fulfillment__la_fulfilment_fact ff ON (order_tracking.order_item_unit_shipment_id = ff.shipment_merchant_reference_id)
               LEFT OUTER JOIN
                 (SELECT product_categorization_hive_dim_key,
                         product_id,
                         analytic_super_category,
                         brand,
                         analytic_vertical,
                         analytic_category,
                         analytic_sub_category
                  FROM bigfoot_external_neo.sp_product__product_categorization_hive_dim) prod ON order_tracking.product_categorization_hive_dim_key = prod.product_categorization_hive_dim_key
               LEFT OUTER JOIN bigfoot_external_neo.scp_ekl__la_shipment_fact sh ON order_tracking.order_item_unit_shipment_id = sh.merchant_reference_id
               LEFT OUTER JOIN
                 (SELECT DISTINCT `data` . group_id AS last_group_id,
                                  `data` . cutoff AS last_conn_cutoff_in_sec
                  FROM bigfoot_snapshot.dart_wsr_scp_ekl_connection_1_11_view_total) last_conn ON last_conn.last_group_id = sh.shipment_last_consignment_conn_id
               LEFT OUTER JOIN
                 (SELECT vendor_tracking_id,
                  ...
                  FROM bigfoot_external_neo.scp_ekl__la_tpt_fact
                  GROUP BY vendor_tracking_id) TPT ON sh.vendor_tracking_id = TPT.vendor_tracking_id
               LEFT OUTER JOIN
                 (SELECT vendor_tracking_id,
                         max(MH_breach_flag) AS mh_breach_flag,
                         max(MH_promise_dateTime) AS mh_promise_datetime
                  FROM bigfoot_external_neo.scp_ekl__mh_breach_large_fact
                  GROUP BY vendor_tracking_id) MH_breach ON sh.vendor_tracking_id = MH_breach.vendor_tracking_id
               LEFT OUTER JOIN
                 (SELECT DISTINCT `data` . cancellation_reason AS cancellation_reason,
                                  `data` . cancellation_sub_reason AS cancellation_sub_reason,
                                  cancel_order_item_id
                  FROM bigfoot_snapshot.dart_fkint_scp_oms_cancellation_0_3_view_total LATERAL VIEW explode(`data` . cancellation_items.cancellation_item_order_item_id) cancel_order_item_id AS cancel_order_item_id) CANCEL ON order_tracking.order_item_id = cancel.cancel_order_item_id
               LEFT OUTER JOIN bigfoot_external_neo.scp_fulfillment__cancellation_reason_hive_dim cancel_Dim ON (cancel.cancellation_reason = cancel_Dim.cancellation_reason
                 ...
                 (SELECT tracking_id,
                  ...
                  FROM
                    (SELECT return_shipment_id,
                            (LAST_VALUE(return_reason) OVER(PARTITION BY return_shipment_id
                                                            ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_reason,
                            (LAST_VALUE(return_derived_reason) OVER(PARTITION BY return_shipment_id
                                                                    ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_derived_reason,
                            (LAST_VALUE(return_id) OVER(PARTITION BY return_shipment_id
                                                        ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_id,
                            (LAST_VALUE(return_sub_reason) OVER(PARTITION BY return_shipment_id
                                                                ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_sub_reason,
                            (LAST_VALUE(return_comments) OVER(PARTITION BY return_shipment_id
                                                              ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_comments,
                            (LAST_VALUE(order_item_id) OVER(PARTITION BY return_shipment_id
                                                            ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS order_item_id,
                            (LAST_VALUE(return_type) OVER(PARTITION BY return_shipment_id
                                                          ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_type,
                            (LAST_VALUE(return_date) OVER(PARTITION BY return_shipment_id
                                                          ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS return_date,
                            (LAST_VALUE(exchange_fsn) OVER(PARTITION BY return_shipment_id
                                                           ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS exchange_fsn,
                            (LAST_VALUE(replacement_order_item_id) OVER(PARTITION BY return_shipment_id
                                                                        ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS replacement_order_item_id,
                            (LAST_VALUE(tracking_id) OVER(PARTITION BY return_shipment_id
                                                          ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) AS tracking_id,
                            ROW_NUMBER() OVER(PARTITION BY return_shipment_id
                                              ORDER BY a.updateat ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) row_num
                     FROM
                       (SELECT return_item. `data` . reason AS return_reason,
                        ...
                        FROM bigfoot_snapshot.dart_fkint_scp_rrr_return_item_3_0_view_total return_item
                        LEFT OUTER JOIN bigfoot_snapshot.dart_fkint_scp_rrr_return_4_0_view_total RETURN ON return_item. `data` . return_id_string = RETURN. `data` . return_id_string
                        LEFT OUTER JOIN bigfoot_common.courier_return_reason_dim_alter return_reason_dim ON return_item. `data` . reason = return_reason_dim.courier_return_item_reason
                        WHERE return_item.`data`.return_item_status != 'cancelled') a) RETURN
                  WHERE row_num = 1 ) rrr_return ON order_tracking.order_item_id = rrr_return.r_order_item_id
               LEFT OUTER JOIN
                 (SELECT max(qc.qc_verification_reason) AS qc_reason,
                         shi.shipment_display_id AS sh_merchant_reference_id
                  FROM bigfoot_external_neo.scp_warehouse__fc_shipment_item_l0_hive_fact shi
                  LEFT JOIN bigfoot_external_neo.scp_warehouse__fc_qc_ticket_item_l0_hive_fact qc ON shi.shipment_item_id = qc.shipment_item_id
                  AND shi.warehouse_company = qc.warehouse_company
                  WHERE shipment_entity_type = 'shipment'
                    AND shipment_is_large_warehouse = 1
                  GROUP BY shi.shipment_display_id) qc_wms ON sh.merchant_reference_id = qc_wms.sh_merchant_reference_id
               LEFT OUTER JOIN bigfoot_external_neo.scp_warehouse__fc_reservation_l0_hive_fact pl ON SUBSTR(pl.reservation_fulfill_reference_id, 5) = ff.fulfill_item_unit_id
               AND pl.reservation_inv_source_type IN('fki',
                                                     'wsr',
                                                     'fbf')
               AND pl.reservation_outbound_type = 'customer_reservation') A) B) C) DA) E;