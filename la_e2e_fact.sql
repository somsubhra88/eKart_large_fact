INSERT overwrite TABLE la_e2e_fact 
SELECT
			order_external_id,
			order_id,
			order_item_id,
				...
				SELECT
					...
					(
						CASE
							...
							WHEN isnull(shipment_first_ofd_datetime)
							AND isnull(tpl_first_ofd_time) THEN '06_DeliveryHub'
					...
					IF(
						...
						IF(
							...
							'Customer_Dependency-Attempted Undelivered',
							IF(
						...
						IF(
							...
							IF(
								...
								IF(
									...
									OR to_date(tpl_first_ofd_time) <= to_date(perfect_order_final_promised_date),
									0,
									IF(
					...
					CASE WHEN shipment_breach_bucket != 'No Breach-OFD Before LPD' THEN fk_triggered_promise_change_flag ELSE 0 END AS fk_triggered_promise_change_flag,
					fk_triggered_cancellation_flag,
					fk_triggered_return_flag,
					IF(
						(
					...
					CASE
						...
						AND shipment_breach_bucket NOT IN(
							...
							'No Breach-OFD Before LPD',
							'Customer_Dependency-Attempted',
							'Customer_Dependency-Attempted Undelivered-3PL'
						) THEN 1
					...
					CASE
						...
						AND shipment_breach_bucket != 'No Breach-OFD Before LPD' THEN 1
						ELSE 0
				...
				FROM
					(
						SELECT
							...
							C.tpl_first_ofd_date_key,
							C.tpl_last_ofd_date_key,
						...
						FROM
							(
								SELECT
									...
									lookup_date(
										to_date(tpl_first_ofd_time)
									) AS tpl_first_ofd_date_key,
									lookup_date(
										to_date(tpl_last_ofd_time)
									) AS tpl_last_ofd_date_key,
									lookup_date(
								...
								FROM
									(
										SELECT
											...
											fulfill_item_unit_final_reserved_datetime,
											lookup_date(
												to_Date(fulfill_item_unit_final_reserved_datetime)
											) AS fulfill_item_unit_final_reserved_date_key,
											A.seller_id,
											A.payment_mode,
											A.pos_id,
											A.transaction_id,
										...
										FROM
											(
												SELECT
													...
													sh.ekl_billable_weight,
													sh.tpl_first_ofd_time,
													sh.tpl_last_ofd_time,
													...
													sh.dummy1 AS pincode_location_type,
													sh.dummy2 AS is_cpu_vendor,
												...
												LEFT OUTER JOIN(
														...
														FROM
															bigfoot_snapshot.dart_wsr_scp_ekl_connection_1_11_view_total
													) last_conn ON
													last_conn.last_group_id = sh.shipment_last_consignment_conn_id
												LEFT OUTER JOIN(
														SELECT
															vendor_tracking_id,
															max( tpt_intransit_breach_flag ) AS tpt_intransit_breach_flag,
															max( shipment_tpt_dh_eta_datetime ) AS shipment_tpt_dh_eta_datetime
														FROM
												...
												LEFT OUTER JOIN(
														...
														FROM
															bigfoot_snapshot.dart_fkint_scp_oms_cancellation_0_3_view_total LATERAL VIEW explode(
															...
															(
																...
																FROM
																	(
																		...
																		FROM
																			bigfoot_snapshot.dart_fkint_scp_rrr_return_item_3_0_view_total return_item
																		LEFT OUTER JOIN bigfoot_snapshot.dart_fkint_scp_rrr_return_4_0_view_total RETURN ON
