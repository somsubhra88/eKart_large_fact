				...
				FROM
					(
						SELECT
							...
							C.shipment_last_consignment_conn_id,
						...
						FROM
							(
								SELECT
									...
									lookup_date(to_date(B.shipment_tpt_dh_eta_datetime)) AS shipment_tpt_dh_eta_date_key,
									if(B.fsd_first_dh_received_datetime IS NOT NULL, if(B.fsd_first_dh_received_datetime<=B.shipment_tpt_dh_eta_datetime,0,1),if(from_unixtime(unix_timestamp())>B.shipment_tpt_dh_eta_datetime,1,0)) AS tpt_intransit_breach_flag
								FROM
									(
										SELECT
											...
											shipment_created_at_datetime,
											A.shipment_last_consignment_conn_id,
											shipment_last_consignment_create_datetime,
											IF(
											...
											CASE 
												WHEN A.fulfill_item_unit_is_for_slotted_delivery='NotSlotted' THEN to_utc_timestamp(concat(substr(A.fulfill_item_unit_delivered_status_expected_date_key,1,4),'-',substr(A.fulfill_item_unit_delivered_status_expected_date_key,5,2),'-',substr(A.fulfill_item_unit_delivered_status_expected_date_key,7,2),' 06:00:00'),'')
												WHEN A.fulfill_item_unit_is_for_slotted_delivery='Slotted' 
												THEN 
												to_utc_timestamp(from_unixtime(IF(unix_timestamp(A.shipment_last_consignment_eta_datetime) >=
												unix_timestamp(A.dispatch_max_time) + 7200 + unix_timestamp(A.shipment_last_consignment_eta_datetime) - unix_timestamp(A.shipment_first_consignment_create_datetime),
												unix_timestamp(A.shipment_last_consignment_eta_datetime),
												unix_timestamp(A.dispatch_max_time) + 7200 + unix_timestamp(A.shipment_last_consignment_eta_datetime) - unix_timestamp(A.shipment_first_consignment_create_datetime)
												)),'')
											...
											A.shipment_first_consignment_create_datetime
										FROM
											(
												SELECT
													...
													if(ff.fulfill_item_unit_dispatch_expected_time>ff.fulfill_item_unit_dispatch_actual_time,fulfill_item_unit_dispatch_expected_time, fulfill_item_unit_dispatch_actual_time) AS dispatch_max_time,
													...
													sh.shipment_last_consignment_eta_datetime,
													sh.shipment_first_consignment_create_datetime,
