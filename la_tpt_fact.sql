...
FROM
  (SELECT Sh.fsd_assigned_hub_id_key AS fsd_assigned_hub_id_key,
   ...
   FROM
     (SELECT fulfill_item_unit_dispatch_expected_time,
      ...
      FROM bigfoot_external_neo.scp_fulfillment__la_fulfilment_fact
      WHERE fulfill_item_unit_size='Large') ff
   INNER JOIN
     (SELECT merchant_reference_id,
             ...
             ekl_shipment_type
      FROM bigfoot_external_neo.scp_ekl__la_shipment_fact
      WHERE shipment_last_consignment_id IS NOT NULL) Sh ON ff.shipment_merchant_reference_id = Sh.merchant_reference_id
   LEFT JOIN
     (SELECT ekl_facility_hive_dim_key,
             facility_id AS origin_facility_id
      FROM bigfoot_external_neo.scp_ekl__ekl_facility_hive_dim) Origin ON Sh.shipment_origin_facility_id_key = Origin.ekl_facility_hive_dim_key
   LEFT JOIN
     (SELECT ekl_facility_hive_dim_key AS dest_fid_dim_key,
             facility_id AS dest_facility_id
      FROM bigfoot_external_neo.scp_ekl__ekl_facility_hive_dim) Dest ON Sh.fsd_assigned_hub_id_key = Dest.dest_fid_dim_key
   LEFT JOIN
     (SELECT fkl_facility_id,
             mh_facility_id
      FROM bigfoot_common.ekl_fkl_facility_mother_hub_mapping) B ON B.fkl_facility_id = Origin.origin_facility_id
   WHERE not(isnull(Sh.vendor_tracking_id))
     AND not(isnull(Dest.dest_facility_id))
     AND not(isnull(Origin.origin_facility_id))) A;